...
# Custom commands exposed as `devctl <name>`
# Handlers are executed from the project root and should be executable
# Paths can be executable scripts or directories with index.js/index.ts
# TypeScript handlers require `tsx` or `ts-node` in the project
# TypeScript handlers can import `CustomCommandPayload` from @maktouch/devctl
# [optional]
commands:
  - name: setup-ssl
  ...
  - name: setup-hosts
    description: Add local development domains to /etc/hosts (requires sudo)
    handler: .devctl/commands/setup-hosts
